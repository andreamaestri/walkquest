"""Models for walks, adventures, companions, and related functionality."""
import uuid
...
class Adventure(models.Model):
    ...
    def duration(self):
        ...
        Returns:
            timedelta: Duration of the adventure, or None if times not set
        """
        if self.start_time and self.end_time:
            # Use timezone.now() instead of datetime.today() for timezone awareness
            now = timezone.now()
            ...
            start = timezone.make_aware(
                ...
                datetime.combine(now.date(), self.end_time),
            )
            return end - start
        return None
...
class Walk(models.Model):
    ...
    def get_steepness(self):
        """Get steepness level from adventure."""
        return self.adventure.difficulty_level if self.adventure else None

    def save(self, *args, **kwargs):
        """Save walk with auto-generated fields."""
    ...
    def is_favorite_of(self, user) -> bool:
        ...
        Returns:
            bool: True if the walk is favorited by the user
        """
...
class WalkFavorite(models.Model):
    ...
    def __str__(self):
        ...
        return f"{self.user.username}'s favorite: {self.walk.walk_name}"
