<template>
  <Transition
    ...
    :css="false"
  >
    <div class="fixed inset-0 bg-white flex flex-col z-10">
        ...
        <div class="flex-1 overflow-y-auto p-4 md:p-6 space-y-6">
            ...
            <div class="flex flex-wrap gap-4">
              <button class="flex items-center gap-2 px-4 py-2 h-12 bg-purple-600 text-white rounded-2xl hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-purple-500">
            ...
            <section v-if="walk.points_of_interest" class="space-y-3">
              ...
              <span v-for="poi in parsedPOIs" :key="poi"
            ...
            <section v-if="walk.features?.length || walk.categories?.length" class="space-y-3">
                ...
                <div class="flex flex-wrap gap-2">
                    <span v-for="feature in walk.features" 
                          :key="feature.name"
                          class="px-3 py-1.5 bg-gray-100 rounded-full text-sm">
                        {{ feature.name }}
                    </span>
                    <span v-for="category in walk.categories" 
                          :key="category.name"
                          class="px-3 py-1.5 bg-gray-100 rounded-full text-sm">
                        {{ category.name }}
                    </span>
...
import { MD3_EASING, MD3_DURATION, createMotion } from '../utils/motion'
...
<style scoped>
@import "tailwindcss";
/* Remove the old keyframes animation since we're using motion */
</style>
